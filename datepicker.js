// непонятно, почему нет точек с запятой!

let datepickers = [] // Get's reassigned in `remove()` below.
const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
const months = [ 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December' ]
const sides = { t: 'top', r: 'right', b: 'bottom', l: 'left', c: 'centered' }
...
function datepicker(selector, options) {
  ...
  const instance = createInstance(selector, options)// Create the datepicker instance!
 ...
 */
function applyListeners() { events.forEach(event => document.addEventListener(event, oneHandler)) }
...
function createInstance(selector, opts) {
  ...
  const instance = {
    el,    // The calendar will be positioned relative to this element (except when 'body').
    parent,// The element that datepicker will be child of in the DOM.
    nonInput: el.nodeName !== 'INPUT',// Indicates whether to use an <input> element or not as the calendar's anchor.
    noPosition,// Flag indicating if `el` is 'body' for `calculatePosition`.
    position: noPosition ? false : options.position,// Calendar position relative to `el`.
    startDate,// Date obj used to indicate what month to start the calendar on.
    dateSelected,// Starts the calendar with a date selected.
    disabledDates: options.disabledDates,// An array of dates to disable.
    minDate: options.minDate,// Low end of selectable dates.
    maxDate: options.maxDate,// High end of selectable dates.
    noWeekends: !!options.noWeekends,// Disabled the ability to select days on the weekend.
    weekendIndices: options.weekendIndices,// Indices for "Saturday" and "Sunday" repsectively.
    calendar,// The element our calendar is constructed in.
    currentMonth: (startDate || dateSelected).getMonth(),// Month of `startDate` or `dateSelected` (as a number).
    currentMonthName: (options.months || months)[(startDate || dateSelected).getMonth()],// Month name in plain english - or not.
    currentYear: (startDate || dateSelected).getFullYear(),// Year of `startDate` or `dateSelected`.
    setDate,// Method to programmatically set the calendar's date.
    remove,// Method that removes the calendar from the DOM along with associated events.
    setMin,// Method to programmatically change the minimum selectable date.
    setMax,// Method to programmatically change the maximum selectable date.
    show,// Method to programmatically show the calendar.
    hide,// Method to programmatically hide the calendar.
    onSelect: options.onSelect,// Callback fired when a date is selected - triggered in `selectDay`.
    onShow: options.onShow,// Callback fired when the calendar is shown - triggered in `showCal`.
    onHide: options.onHide,// Callback fired when the calendar is hidden - triggered in `hideCal`.
    onMonthChange: options.onMonthChange,// Callback fired when the month is changed - triggered in `changeMonthYear`.
    formatter: options.formatter,// Function to customize the date format updated on <input> elements - triggered in `setCalendarInputValue`.
    disabler: options.disabler,// Function with custom logic that determines wether a given date is disabled or not.
    months: options.months || months,// Labels for months - custom or default.
    days: options.customDays || days,// Labels for days - custom or default.
    startDay: options.startDay,// Start day of the week - indexed from `days` above.
    overlayPlaceholder: options.overlayPlaceholder || '4-digit year',// Custom overlay placeholder.
    overlayButton: options.overlayButton || 'Submit',// Custom overlay submit button.
    disableYearOverlay: !!options.disableYearOverlay,// Disable the overlay for changing the year.
    ...
    isMobile: 'ontouchstart' in window,// Used in conjuntion with `disableMobile` above within `oneHandler`.
    alwaysShow: !!options.alwaysShow,// Prevents the calendar from hiding.
      id: options.id}// Used to connect 2 datepickers together to form a daterange picker.
